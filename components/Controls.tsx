...
import { Play, Pause, RotateCcw, Swords, Shield, Crosshair } from 'lucide-react';
      ...
      <div className="flex flex-1 justify-center gap-6 items-center">
        ...
        <div className="flex bg-neutral-800 rounded-lg p-1">
           <button 
             ...
             className={`px-4 py-2 rounded-md font-bold transition-all ${selectedTeam === Team.RED ? 'bg-red-600 text-white' : 'text-neutral-400 hover:text-white'}`}
           >
             RED
           </button>
           <button 
             onClick={() => setSelectedTeam(Team.BLUE)}
             className={`px-4 py-2 rounded-md font-bold transition-all ${selectedTeam === Team.BLUE ? 'bg-blue-600 text-white' : 'text-neutral-400 hover:text-white'}`}
           >
             BLUE
           </button>
        </div>
        ...
        <div className="flex gap-2">
            {[
                { type: UnitType.SOLDIER, icon: Swords, label: 'Soldier' },
                { type: UnitType.TANK, icon: Shield, label: 'Tank' },
                { type: UnitType.ARCHER, icon: Crosshair, label: 'Archer' }
            ].map((u) => (
                <button
                    key={u.type}
